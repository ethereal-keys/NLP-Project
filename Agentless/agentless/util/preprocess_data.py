...
PROJECT_FILE_LOC = "/scratch/eunwooim/Agentless/data"
...
def get_repo_structure(instance_id: str, repo_name, base_commit, playground):
    if PROJECT_FILE_LOC is not None:
        ...
        if "structure" in d:
            ...
            print("Repo structure exists in the json file")
        else:
            d = get_project_structure_from_scratch(repo_name, base_commit, instance_id, playground)
            with open(PROJECT_FILE_LOC + "/" + instance_id + ".json", "w") as f:
                json.dump(d, f, indent=4)
                print(f"Overwrote {PROJECT_FILE_LOC + '/' + instance_id + '.json'}")
            repo_structure = d["structure"]
    else:
