...
class OpenSourceDecoder(DecoderBase):
    def __init__(self, name: str, logger, **kwargs) -> None:
        super().__init__(name, logger, **kwargs)
        self.name = name
    ...
    def codegen(
        ...
        for _ in range(num_samples):
            ...
            with torch.inference_mode():
                output = model.generate(**input_ids, max_new_tokens=4096, cache_implementation="static")
                out_txt = tokenizer.decode(output[0], skip_special_tokens=True).replace(message, "").strip()
                ...
                trajs.append(
                    {
                        ...
                        "usage": {
                            ...
                            "prompt_tokens": len(input_ids['input_ids']),
                        },
                    }
                )
            else:
