    ...
    def retrieve(self, mock=False):
        ...
        if not os.path.exists(persist_dir) or mock:
            ...
            if mock:
                ...
                # embed_model = make_model(
                #     model="meta-llama/CodeLlama-7b-Instruct-hf",
                #     backend="opensource",
                #     logger=self.logger,
                # )
                from llama_index.embeddings.huggingface import HuggingFaceEmbedding
                # embed_model = HuggingFaceEmbedding(model_name="meta-llama/CodeLlama-7b-Instruct-hf")
                embed_model = HuggingFaceEmbedding(model_name="sentence-transformers/all-MiniLM-L6-v2")
            # index = VectorStoreIndex.from_documents(documents, embed_model=embed_model)
            # vector_index = index.as_query_engine(llm=embed_model).query('')
            index = VectorStoreIndex.from_documents(documents, embed_model=embed_model)
